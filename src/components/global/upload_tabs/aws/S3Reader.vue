<template>
  <v-stepper v-model="step" vertical class="elevation-0">
    <v-stepper-step :complete="!!assumed_role" step="1">
      <div>
        Account Credentials
        <AWSHelpModal v-model="help_dialogue" />
...
export default class S3Reader extends Props {
  ...
  get shown_error(): string {
    if (!this.error) {
      return '';
    } else if ((this.error + '').includes('Networking error')) {
      return `${this.error} Your CORS configuration may also be invalid. (See steps 5 and 7 in help)`;
    } else {
